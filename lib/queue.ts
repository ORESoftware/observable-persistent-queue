...
import {handlePriority} from './handle-priority';
import {startTail} from './start-tail';
import {
  IPriority, IPriorityInternal, IQueueBuilder,
  IDequeueOpts, IDrainOpts, IEnqueueOpts
...
export class Queue extends QProto {
  ...
  eqStream(pauser?: Subject<any>, opts?: any): Observable<any> {
    ...
    .catch(e => {
      log.error('error in dequeueStream method => ', e.stack || e);
      const force = !String(e.stack || e).match(/acquire lock timed out/);
      return releaseLock(this, force);
    });
  ...
  isEmpty(obs: Subject<any>): Observable<any> {
    ...
    .map(() => {
      log.info('isEmpty is true.');
      // obs.complete();
      return {isEmpty: true}
    })
    .catch(e => {
      log.error('isEmpty() error:', e.stack || e);
      const force = !String(e.stack || e).match(/acquire lock timed out/);
      return releaseLock(this, force);
    })
    .take(1);
  ...
  enq(lines: string | Array<string>, opts?: IEnqueueOpts): Observable<any> {
    ...
    .catch(err => {
      log.error('add / enqueue error:', err.stack || err);
  ...
  deq(opts: IDequeueOpts): Observable<any> {
    ...
    const isPriority = this.priority ? (opts.isPriority !== false) : (opts.isPriority === true);
