...
exports.countLines = function (q, pattern) {
    return count_lines_1.getLineCountOfQueue(q.fp, pattern);
};
exports.findFirstLine = function (q, pattern) {
    pattern = pattern || '\\S+';
    var count = 1;
    return sed_1.sed(q, pattern, false, count).map(function (data) {
        return data[0];
    });
};
exports.removeOneLine = function (q, pattern) {
    pattern = pattern || '\\S+';
    var count = 1;
    return sed_1.sed(q, pattern, true, count).map(function (data) {
        if (data.length > 1) {
...
exports.ifFileExistAndIsAllWhiteSpaceThenTruncate = function (q) {
    return exports.readFile(q).flatMap(function (data) {
        if (data) {
...
exports.acquireLockRetry = function (q, obj) {
    ...
    return Rx_1.Observable.interval(1500).takeUntil(Rx_1.Observable.race(exports.acquireLock(q, obj.name).filter(function (obj) { return !obj.error; }), Rx_1.Observable.timer(3600).flatMap(function () {
...
exports.waitForClientCount = function (q, opts) {
    ...
    var index = 0;
    return q.clientStream.bufferCount(count).filter(function (value) {
        index++;
        var first = value[0];
