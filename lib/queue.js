...
class Queue extends queue_proto_1.QProto {
    constructor(obj) {
        ...
        this.isEmptyStream = new rxjs_2.Subject();
        this.obsDequeue = new rxjs_2.Subject();
        let index = 0;
        let obsEnqueue = this.obsEnqueue = new rxjs_2.Subject();
        this.queueStream = rxjs_1.Observable.create(obs => {
        ...
        this.isReady = false;
        let callable = true;
        let obsClient = this.obsClient = new rxjs_2.Subject();
    ...
    eqStream(pauser, opts) {
        if (!(pauser instanceof Rx.Observable)) {
            opts = pauser || {};
            pauser = new rxjs_2.Subject();
        }
        opts = opts || {};
        let $obs = rxjs_1.Observable.zip(this.queueStream, pauser);
