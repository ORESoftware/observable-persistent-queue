...
var client_1 = require("live-mutex/client");
var lmUtils = require("live-mutex/utils");
var logging_1 = require("./logging");
var queue_proto_1 = require("./queue-proto");
var handle_priority_1 = require("./handle-priority");
var start_tail_1 = require("./start-tail");
...
var Queue = (function (_super) {
    ...
    function Queue(obj) {
        ...
        var lck = _this.lock;
        if (obj.priority) {
            handle_priority_1.handlePriority(obj, _this);
        }
        _this.isEmptyStream = new Rx_1.Subject();
        _this.obsDequeue = new Rx_1.Subject();
        var index = 0;
        var obsEnqueue = _this.obsEnqueue = new Rx_1.Subject();
        _this.queueStream = Rx_1.Observable.create(function (obs) {
            var push = Rx_1.Subscriber.create(function (v) {
        ...
        _this.isReady = false;
        var callable = true;
        var obsClient = _this.obsClient = new Rx_1.Subject();
        var clientEE = new EE().setMaxListeners(200);
        var opqId = 0;
        _this.clientStream = Rx_1.Observable.create(function (sub) {
            var push = Rx_1.Subscriber.create(function (v) {
        ...
        _this.init = function (isPublish) {
            ...
            callable = false;
            var promise = lmUtils.launchBrokerInChildProcess({ port: port, detached: true });
            return Rx_1.Observable.fromPromise(promise)
                .flatMap(function () {
                _this.client = new client_1.Client({ key: lck, port: port, listener: onClientConnectionChange });
                return Rx_1.Observable.fromPromise(_this.client.ensure());
            })
                .flatMap(function () {
                ...
                .flatMap(function (obj) {
                return genericAppendFile(_this, '').map(function () { return obj; });
            })
                .flatMap(function (obj) {
                return ifFileExistAndIsAllWhiteSpaceThenTruncate(_this).map(function () { return obj; });
            })
                .flatMap(function (obj) {
                ...
                clientEE.emit('ready');
                _this.isReady = true;
                start_tail_1.startTail(_this, push);
            })
                .take(1)
                .catch(function (e) {
    ...
    Queue.prototype.eqStream = function (pauser, opts) {
        var _this = this;
        if (!(pauser instanceof Rx_1.Observable)) {
            opts = pauser || {};
            pauser = new Rx_1.Subject();
        }
        opts = opts || {};
        ...
        return $obs
            ...
            .catch(function (e) {
            logging_1.log.error('error in dequeueStream method => ', e.stack || e);
            var force = !String(e.stack || e).match(/acquire lock timed out/);
            return releaseLock(_this, force);
        });
    };
    ...
    Queue.prototype.isNotEmpty = function (obs) {
        ...
        return obs
            ...
            .map(function () {
            logging_1.log.info('Queue is *not* empty.');
    ...
    Queue.prototype.isEmpty = function (obs) {
        ...
        return obs
            ...
            .map(function () {
            logging_1.log.info('isEmpty is true.');
            return { isEmpty: true };
        })
            .catch(function (e) {
            logging_1.log.error('isEmpty() error:', e.stack || e);
            var force = !String(e.stack || e).match(/acquire lock timed out/);
            return releaseLock(_this, force);
        })
            .take(1);
    ...
    Queue.prototype.drain = function (obs, opts) {
        var _this = this;
        if (!(obs instanceof Rx_1.Subject)) {
            opts = obs || {};
            obs = new Rx_1.Subject();
        }
        opts = opts || {};
    ...
    Queue.prototype.enq = function (lines, opts) {
        ...
        var $add = this.init()
            ...
            .catch(function (err) {
            logging_1.log.error('add / enqueue error:', err.stack || err);
    ...
    Queue.prototype.deq = function (opts) {
        ...
        var isPriority = this.priority ? (opts.isPriority !== false) : (opts.isPriority === true);
        ...
        var $dequeue = this.init()
            ...
            .flatMap(function (obj) {
            return removeMultipleLines(_this, pattern, count).map(function (lines) { return ({ lines: lines, id: obj.id }); });
        })
            .flatMap(function (obj) {
            return releaseLock(_this, obj.id).map(function () { return obj.lines; });
        })
            .catch(function (e) {
            logging_1.log.error(e.stack || e);
            var force = !String(e.stack || e).match(/acquire lock timed out/);
